...
describe('Neo4j Connection Integration', () => {
  ...
  it('should create a node and retrieve it', async () => {
    ...
    // Retrieve the node
    const result = await runQuery<{p: any}>(
    ...
    expect(result).toHaveLength(1);
    expect(result[0].p.properties.name).toBe('Alice');
    expect(result[0].p.properties.age.toNumber()).toBe(30);
    
    // Clean up
    await runQuery('MATCH (p:Person {name: $name}) DELETE p', { name: 'Alice' });
  });
  ...
  it('should handle transactions', async () => {
    ...
    try {
      ...
      // Clean up
      await session.run('MATCH (p:Person {name: $name}) DELETE p', { name: 'Bob' });
    } finally {
      // Close session
      await session.close();
    }
